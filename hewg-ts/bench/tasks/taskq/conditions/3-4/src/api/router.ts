/**
 * @hewg-module taskq/api/router
 *
 * Top-level API router that authenticates incoming requests and
 * dispatches them to the appropriate handler based on method and path.
 */
 ...
 * Performs JWT authentication and tenant context resolution before
...
export class ApiRouter {
  ...
  /**
   * Route a request to the appropriate handler based on method and
   * path. Authenticates the request using JWT tokens verified against
   * the configured JWKS endpoint. Health and readiness endpoints
   * bypass authentication.
   *
   * @hewg-module taskq/api/router
   * @effects fs.read, fs.write, net.https, log, time.read
   */
  async route(
    method: string,
    path: string,
