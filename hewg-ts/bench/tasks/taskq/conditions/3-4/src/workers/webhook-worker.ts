/**
 * @hewg-module taskq/workers/webhook-worker
 *
 * Worker that processes jobs by sending webhook POST requests
 * to configured URLs.
 */
...
/** Payload shape expected by the webhook worker. */
...
/** A webhook worker extends BaseWorker with webhook-specific processing. */
export interface WebhookWorker extends BaseWorker {
  readonly workerKind: "webhook";
}

/**
 * Create a webhook worker that processes jobs by POSTing to URLs.
 * @hewg-module taskq/workers/webhook-worker
 * @effects net.https, log, time.read
 */
...
/**
 * Process a single job by sending a webhook POST to the target URL.
 * The payload is sent as JSON with a content-type header.
 * @hewg-module taskq/workers/webhook-worker
 * @effects net.https, log, time.read
 */
